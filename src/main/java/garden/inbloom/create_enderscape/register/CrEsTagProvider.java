...
public class CrEsTagProvider {
	
	public class CrEsBlockTagProvider extends BlockTagsProvider {

		public CrEsBlockTagProvider(PackOutput output, CompletableFuture<Provider> lookupProvider, String modId,
		...
		protected void addTags(Provider provider) {
			...
			for (Block block : connectsToCopperRoof) {
				tag(CrEsBlockTags.CONNECTS_TO_ROOF_BLOCKS.tag).add(block);
				tag(CrEsBlockTags.CONNECTS_TO_ROOF_BLOCKS_COPPER.tag).add(block);
			}
			...
			for (Block block : connectsToShadolineRoof) {
				tag(CrEsBlockTags.CONNECTS_TO_ROOF_BLOCKS.tag).add(block);
				tag(CrEsBlockTags.CONNECTS_TO_ROOF_BLOCKS_SHADOLINE.tag).add(block);
			}
			
			tag(CrEsBlockTags.ROOF_BLOCKS.tag).add(CrEsBlocks.SHADOLINE_SHINGLES.get(), CrEsBlocks.SHADOLINE_TILES.get());
		    ...
		    for (CopperBlockSet.Variant<?> variant : shingles.getVariants()) {
		        for (boolean waxed : new boolean[]{false, true}) {
		            for (WeatherState state : WeatherState.values()) {
		                ...
		                if (entry != null) {
		                    tag(CrEsBlockTags.ROOF_BLOCKS.tag).add(entry.get());
		                    tag(CrEsBlockTags.ROOF_BLOCKS_COPPER.tag).add(entry.get());
		                }
		            }
		        }
		    }
